...
const mediaRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get<{ Params: { bunnyVideoId: string } }>(
    "/video-assets/:bunnyVideoId/playback",
    async (request) => {
      ...
      const response: BunnyPlaybackResponse = {
        ...
        embedUrl: libraryId
          ? `https://iframe.mediadelivery.net/embed/${libraryId}/${bunnyVideoId}`
          : null,
  ...
  fastify.post<{ Params: { bunnyVideoId: string } }>(
    "/video-assets/:bunnyVideoId/sync",
    async (request) => {
