...
export default function ChapterBasedCourse({ courseSlug }: { courseSlug: string }) {
  ...
  const _freeChapter = chapters.find((ch) => ch.isFreeIntro);
  const paidChapters = chapters.filter((ch) => !ch.isFreeIntro);
  const standardBundle = bundles.find((b) => b.type === "STANDARD");
  const _premiumBundle = bundles.find((b) => b.type === "PREMIUM");
  const _totalIndividualPrice = paidChapters.reduce((s, ch) => s + ch.price, 0);
  const _totalLessons = chapters.reduce((s, ch) => s + ch.lessonCount, 0);
    ...
    <div className="min-h-screen bg-[#F3F4F6] font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-14 space-y-16">
        ...
        <div id="chapters-list">
          <h2 className="text-3xl font-bold text-[#0F1C15] mb-8 font-['Rajdhani']">
            Course Chapters
          </h2>
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl border-2 transition-all overflow-hidden ${
                    isPurchased ? "border-[#D4AF37] shadow-md" : "border-gray-200"
                  }`}
                >
                  {/* Chapter Header */}
                  <div className="p-6">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex items-start gap-4 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-xl font-bold text-[#0F1C15] font-['Rajdhani'] leading-tight">
                            {chapter.title}
                          </h3>
                          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mt-1">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {Math.floor(chapter.totalDuration / 60)}h {chapter.totalDuration % 60}
                              m
                            </span>
                            {chapter.isFreeIntro && (
                              <span className="px-2 py-0.5 bg-[#D4AF37]/10 text-[#D4AF37] rounded-full text-xs font-bold border border-[#D4AF37]/30">
                                FREE
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        {chapter.isFreeIntro ? (
                          <Link
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-[#D4AF37] text-[#0F1C15] font-bold rounded-xl hover:bg-[#c49d2f] transition-colors text-sm"
                          >
                          ...
                          <div className="flex items-center gap-3">
                            <div className="text-right">
                              <div className="text-xl font-bold text-[#0F1C15]">
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-5 py-2.5 bg-[#D4AF37] text-[#0F1C15] font-bold rounded-xl hover:bg-[#c49d2f] transition-colors text-sm disabled:opacity-60"
                            >
                              {purchasing === chapter.id ? (
                                <div className="w-4 h-4 border-2 border-[#0F1C15] border-t-transparent rounded-full animate-spin" />
                              ) : (
        ...
        <div id="pricing">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-[#0F1C15] mb-3 font-['Rajdhani']">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-2xl border-2 border-gray-200 p-8 hover:shadow-xl transition-shadow">
              <div className="text-center mb-6">
                <h3 className="text-2xl font-bold text-[#0F1C15] mb-1 font-['Rajdhani']">
                  Single Chapter
                </h3>
                <p className="text-sm text-gray-400">Pay as you learn</p>
              </div>
              <div className="text-center mb-6">
                <div className="text-4xl font-bold text-[#0F1C15]">NPR 3,000 - 5,000</div>
                <div className="text-sm text-gray-400 mt-1">Per chapter</div>
              </div>
              <ul className="space-y-3 mb-8 text-sm">
                {[
                  "Free Overview Modules (2)",
                  "One Paid Chapter Access",
            ...
            {/* Complete Bundle */}
            {standardBundle && (
              <div className="bg-gradient-to-br from-[#0F1C15] to-[#1F3325] rounded-2xl border-4 border-[#D4AF37] p-8 text-white relative shadow-2xl">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#D4AF37] text-[#0F1C15] px-4 py-1 text-xs font-bold rounded-full whitespace-nowrap">
                  BEST VALUE
                </div>
                ...
                <div className="text-center mb-6">
                  <div className="text-5xl font-bold text-[#D4AF37]">
                    NPR {standardBundle.bundlePrice.toLocaleString()}
                  </div>
                  <div className="text-sm text-gray-400 line-through mt-1">
                    NPR {standardBundle.originalPrice.toLocaleString()}
                  </div>
                  <div className="inline-block mt-2 px-3 py-1 bg-[#D4AF37] text-[#0F1C15] rounded-full text-sm font-bold">
                ...
                <ul className="space-y-3 mb-8 text-sm">
                  {[
                    "Free Overview Modules (2)",
                    "All 5 Paid Chapters Unlocked",
                    "Video Lessons & Quizzes",
                    "Study Materials",
                    `${standardBundle.features.mockExamCount} Mock Exams`,
                    ...
                    <li key={f} className="flex items-start gap-2">
                      ...
                      <span
                        className={
                          f.includes("All 5") || f.includes("Mock") || f.includes("Certificate")
                            ? "font-bold"
                            : ""
                        }
                      >
                ...
                <button
                  ...
                  className="w-full py-4 bg-[#D4AF37] text-[#0F1C15] font-bold rounded-xl hover:bg-[#c49d2f] transition-colors text-lg shadow-xl disabled:opacity-60"
                >
