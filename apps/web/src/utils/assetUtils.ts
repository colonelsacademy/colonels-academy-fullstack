...
const withBunnyImageParams = (urlValue: string, options: BunnyImageOptions = {}): string => {
  try {
    ...
    const fmt = options.format === false ? undefined : (options.format ?? (options.width ? 'webp' : undefined));
...
export const getOptimizedResponsiveImageUrl = (src: string, options: BunnyImageOptions = {}): string => {
