...
import {
  Award, BookOpen, Calendar, CheckCircle,
  Clock, Play, Target, TrendingUp, Users
} from "lucide-react";
...
const UPCOMING_SESSIONS = [
  { id: 1, topic: "Command writing studio", date: "Apr 3", time: "1:00 PM", instructor: "Col. Thapa", type: "Live" },
  { id: 2, topic: "Case-analysis live drill", date: "Apr 4", time: "12:00 PM", instructor: "DIG Sharma", type: "Live" },
  { id: 3, topic: "Cadet aptitude sprint review", date: "Apr 5", time: "11:30 AM", instructor: "Col. Thapa", type: "Live" },
];
    ...
    return (
      <div className="min-h-screen bg-[#F3F4F6] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/login" className="px-8 py-3 bg-[#D4AF37] text-[#0F1C15] font-bold rounded-xl uppercase tracking-wider">
            HQ Login
          </Link>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 text-center shadow-sm">
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-2 gap-3 text-center">
                <div>
                  <div className="text-xl font-bold text-[#0F1C15] font-['Rajdhani']">{courses.length}</div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Enrolled", value: courses.length, icon: BookOpen, color: "text-blue-600 bg-blue-50" },
                { label: "Progress", value: "62%", icon: TrendingUp, color: "text-emerald-600 bg-emerald-50" },
                { label: "Live Classes", value: UPCOMING_SESSIONS.length, icon: Users, color: "text-purple-600 bg-purple-50" },
                { label: "Missions", value: `${missions.filter(m => m.completed).length}/${missions.length}`, icon: Target, color: "text-amber-600 bg-amber-50" },
              ].map((stat) => (
                <div key={stat.label} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
                  <div className={`w-10 h-10 rounded-xl ${stat.color} flex items-center justify-center mb-3`}>
                    <stat.icon className="w-5 h-5" />
                  </div>
                  <div className="text-2xl font-bold text-[#0F1C15] font-['Rajdhani']">{stat.value}</div>
            ...
            {/* Continue Learning */}
            <div id="courses" className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="font-bold text-[#0F1C15] font-['Rajdhani'] uppercase tracking-wider">Continue Learning</h2>
                <Link href="/courses" className="text-xs text-blue-600 font-bold hover:underline">Browse All</Link>
              </div>
              {loading ? (
                ...
                <div className="p-12 text-center">
                  ...
                  <Link href="/courses" className="mt-4 inline-block px-6 py-2.5 bg-[#0F1C15] text-white font-bold rounded-xl text-sm uppercase tracking-wider">
                    Explore Courses
                  </Link>
                </div>
              ) : (
                    ...
                    <div key={course.slug} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                      <div className="w-16 h-16 rounded-xl bg-[#0F1C15] flex items-center justify-center shrink-0 overflow-hidden">
                        {course.heroImageUrl ? (
                          ...
                          <img src={course.heroImageUrl} alt={course.title} className="w-full h-full object-cover" />
                        ) : (
                          <BookOpen className="w-7 h-7 text-[#D4AF37]" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-[#0F1C15] text-sm truncate">{course.title}</h4>
                        ...
                        <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{course.durationLabel}</span>
            ...
            {/* Live Classes */}
            <div id="classes" className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            {/* Daily Missions */}
            <div id="missions" className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                <h2 className="font-bold text-[#0F1C15] font-['Rajdhani'] uppercase tracking-wider">Daily Missions</h2>
              </div>
              <div className="p-6 space-y-3">
                {missions.map((mission) => (
                  <div key={mission.id}
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${mission.completed ? "bg-emerald-500 border-emerald-500" : "border-gray-300"}`}>
                      {mission.completed && <CheckCircle className="w-4 h-4 text-white fill-white" />}
                    </div>
                    <span className={`text-sm font-medium ${mission.completed ? "line-through text-gray-400" : "text-[#0F1C15]"}`}>
                      {mission.task}
                    </span>
                  </div>
                ))}
