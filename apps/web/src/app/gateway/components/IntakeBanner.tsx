        ...
        <div className="flex items-start gap-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <img src={current.image} alt={current.label} decoding="async" className="w-full h-full object-contain drop-shadow-lg" />
            </motion.div>
          </AnimatePresence>

          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-bold text-gray-500 tracking-[0.2em] uppercase">{current.label}</span>
            </div>
            <AnimatePresence mode="wait">
              <motion.div
                key={current.id}
        ...
        <div className="flex items-center justify-center gap-6 lg:gap-10">
          {[
            ...
            <div key={item.key} className="flex items-center gap-6 lg:gap-10">
              {i > 0 && <ArrowRight className="w-5 h-5 text-gray-200 hidden sm:block flex-shrink-0" />}
              <div className="text-center min-w-[80px]">
                ...
                <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-1.5">{item.label}</div>
              </div>
            </div>
          ))}
        </div>
