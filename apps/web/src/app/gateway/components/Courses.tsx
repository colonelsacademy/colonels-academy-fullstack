        ...
        {courses.map((course, index) => {
          ...
          return (
            ...
            >
              <TiltCard
                onClick={() =>
                  isEnrolled ? router.push(`/classroom/${course.id}`) : onCourseClick?.(course)
                }
              ...
              >
                ...
                <div className="flex flex-col flex-grow text-left p-1">
                  <h4 className="text-[16px] font-bold text-[#1c1d1f] leading-[1.2] mb-1.5 line-clamp-2 min-h-[40px] font-rajdhani">
                    {course.title}
                  </h4>
                  <p className="text-[12px] text-[#6a6f73] mb-1.5 truncate">{course.instructor}</p>

                  <div className="flex items-center gap-1 mb-1">
                    <span className="text-[14px] font-bold text-[#b4690e]">
                    ...
                    <div className="flex items-center">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star
                          key={s}
                          className={`w-3 h-3 ${s <= Math.floor(course.rating) ? "fill-[#e59819] text-[#e59819]" : "text-gray-200"}`}
                        />
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col mb-3">
                    {course.comingSoon ? (
                      <div className="flex items-center justify-center py-2">
                        <span className="px-3 py-1.5 bg-amber-100 text-amber-800 text-[14px] font-bold rounded-full border border-amber-200">
                          Coming Soon
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[14px] text-[#6a6f73] line-through decoration-gray-400">
                  ...
                  <div className="mt-auto space-y-3">
                    {(course.isBestseller ?? course.isPremium) && !course.comingSoon && (
                      <div className="flex flex-wrap gap-2">
                        {course.isBestseller && (
                          <span className="px-2 py-0.5 bg-[#eceb98] text-[#3d3c0a] text-[10px] font-bold uppercase tracking-wide">
                            Bestseller
                          </span>
                        )}
                        {course.isPremium && (
                          <span className="px-2 py-0.5 bg-[#f3ca8c] text-[#593d00] text-[10px] font-bold uppercase tracking-wide">
                            Premium
                          </span>
                        )}
                      </div>
